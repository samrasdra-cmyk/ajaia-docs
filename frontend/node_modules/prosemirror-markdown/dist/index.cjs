...
var MarkdownSerializerState = function () {
  ...
  _createClass(MarkdownSerializerState, [{
    ...
    value: function renderInline(parent) {
      ...
      var progress = function progress(node, offset, index) {
        ...
        if (node && node.isText && marks.some(function (mark) {
          ...
          return info && info.expelEnclosingWhitespace && !active.some(function (m, i) {
            return i < keep && m.eq(mark);
          });
        })) {
        ...
        if (node && node.isText && marks.some(function (mark, i) {
          var info = _this2.getMark(mark.type.name);
          return info && info.expelEnclosingWhitespace && !_this2.isMarkAhead(parent, index + 1, marks.slice(0, i + 1));
        })) {
        ...
        if (node || index == parent.childCount) while (keep < active.length) _this2.text(_this2.markString(active.pop(), false, parent, index), false);
        ...
        if (node) {
          ...
          _this2.atBlockStart = false;
          if (node.isText && node.nodeSize > 0) _this2.atBlockStart = false;
        }
      };
    ...
    key: "isMarkAhead",
    value: function isMarkAhead(parent, index, marks) {
      for (;; index++) {
