...
export class MarkdownSerializerState {
  ...
  renderInline(parent: Node, fromBlockStart = true) {
    ...
    let progress = (node: Node | null, offset: number, index: number) => {
      ...
      if (node || index == parent.childCount) while (keep < active.length)
      ...
      if (node) {
        ...
        // can we detect that?
        if (node.isText && node.nodeSize > 0) this.atBlockStart = false
      }
    }
  ...
  /// @internal
  isMarkAhead(parent: Node, index: number, marks: readonly Mark[]) {
    for (;; index++) {
